...
## 任务目标
选手需要使用强化学习算法训练智能体，让其控制 GO2 机器狗在仿真环境内探索并学习自主导航与运动控制策略，使得机器狗可以在**未知/半未知**场景中实现**自主寻路**，以尽可能短的时间跨越地形，同时保持运动的稳定性。
...
> **注：** 在创建训练任务和评估任务时，上述地形的配置方式有所不同。具体请查看开发指南-环境配置部分。
...
**总分计算公式：**
$$总分 = 0.4 \times \text{Score}_{\text{forward}} + 0.2 \times \text{Score}_{\text{time}} + 0.2 \times \text{Score}_{\text{energy}} + 0.2 \times \text{Score}_{\text{posture}}$$
...
**总分计算公式：**
$$总分 = \text{完成系数} \times (0.4 \times \text{Score}_{\text{time}} + 0.4 \times \text{Score}_{\text{posture}} + 0.2 \times \text{Score}_{\text{energy}})$$

> **完成系数**为当前任务中完成赛道的机器狗数量占总数的比例：单只机器狗完成赛道记为 1、失败或超时记为 0，对批次内所有机器狗求均值即可。例如并行 1024 只机器狗、其中 512 只走到终点，完成系数 = 0.5。
...
* **Standard 模式走穿判定：** 从地形块中心出生，满足 $\|pos_{\text{current}} - pos_{\text{spawn}}\|_2 \ge L_{\text{terrain}} / 2 - 0.1$（2D 欧氏距离，方向无关）；默认 $L_{\text{terrain}} = 8\text{m}$，阈值约 $3.9\text{m}$
